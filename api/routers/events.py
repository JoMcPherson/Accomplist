...
def create_event(
    event: EventIn, response: Response, repo: eventsRepo = Depends()
):
...
def update_event(
    event_id: int, event: EventIn, repo: eventsRepo = Depends()
...
def get_one_event(
    event_id: int, response: Response, repo: eventsRepo = Depends()
) -> EventOut:
